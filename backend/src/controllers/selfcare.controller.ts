...
export const selfCareLogin = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    const userRecords = await db.select().from(radcheck).where(
      and(
        eq(radcheck.tenantId, data.tenantId),
        eq(radcheck.username, data.username),
...
export const getSelfCareProfile = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const user = request.user as any;
    if (user.role !== "end_user") return reply.code(403).send({ error: "Forbidden" });

    // Get group/profile
    const userGroup = await db.select().from(radusergroup).where(
      and(
        eq(radusergroup.tenantId, user.tenantId),
        eq(radusergroup.username, user.username)
      )
    ).limit(1);
    ...
    if (userGroup.length > 0) {
      ...
      const groupChecks = await db.select().from(radgroupcheck).where(
        and(eq(radgroupcheck.tenantId, user.tenantId), eq(radgroupcheck.groupname, profileName))
      );
      
      const groupReplies = await db.select().from(radgroupreply).where(
        and(eq(radgroupreply.tenantId, user.tenantId), eq(radgroupreply.groupname, profileName))
      );
    ...
    const usageQuery = await db.select({
      ...
      and(
        ...
        eq(radacct.username, user.username)
      )
    );
...
export const getActiveSessions = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    const sessions = await db.select().from(radacct).where(
      and(
        eq(radacct.tenantId, user.tenantId),
        eq(radacct.username, user.username),
...
export const getSessionHistory = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const user = request.user as any;
    if (user.role !== "end_user") return reply.code(403).send({ error: "Forbidden" });
    ...
    ).orderBy(desc(radacct.acctstarttime)).limit(50);
...
export const disconnectSession = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const user = request.user as any;
    if (user.role !== "end_user") return reply.code(403).send({ error: "Forbidden" });
    ...
    const sessionRecords = await db.select().from(radacct).where(
      and(
        eq(radacct.tenantId, user.tenantId),
        eq(radacct.username, user.username),
        eq(radacct.acctsessionid, acctsessionid),
        isNull(radacct.acctstoptime)
      )
    ).limit(1);
    ...
    const nasRecords = await db.select().from(nas).where(
      and(
        eq(nas.tenantId, user.tenantId),
        eq(nas.nasname, nasipaddress) // nasname stores IP
      )
    ).limit(1);
  ...
  try {
    ...
    const userRecords = await db.select().from(radcheck).where(
      and(
        ...
        eq(radcheck.username, user.username),
        eq(radcheck.attribute, "Cleartext-Password"),
        isNull(radcheck.deletedAt)
      )
    ).limit(1);
