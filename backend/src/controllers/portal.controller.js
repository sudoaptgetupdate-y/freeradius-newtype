 ...
 */
export const getPortalSettings = async (request, reply) => {
 ...
 */
export const getPortalSettingsAdmin = async (request, reply) => {
    const { tenantId } = request.params;
    if (!tenantId) {
        return reply.code(400).send({ error: "Tenant ID is required" });
    }
 ...
 */
export const updatePortalSettings = async (request, reply) => {
...
export const registerUser = async (request, reply) => {
    ...
    const existingUser = await db.query.radcheck.findFirst({
        where: and(eq(radcheck.tenantId, tenantId), eq(radcheck.username, body.username)),
    });
    if (existingUser) {
 ...
 */
export const testTelegramSettings = async (request, reply) => {
