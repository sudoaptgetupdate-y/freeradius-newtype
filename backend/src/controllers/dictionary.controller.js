...
export const getDictionary = async (request, reply) => {
    try {
        ...
        if (user.role === "super_admin" || user.role === "admin") {
            ...
            attributes = await db.select().from(radiusDictionary).where(or(isNull(radiusDictionary.tenantId), eq(radiusDictionary.tenantId, user.tenantId)));
        }
...
export const createDictionaryAttribute = async (request, reply) => {
    try {
        const user = request.user || {};
...
export const deleteDictionaryAttribute = async (request, reply) => {
    try {
        const user = request.user || {};
