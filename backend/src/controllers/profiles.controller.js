...
export const updateProfile = async (request, reply) => {
    try {
        ...
        if (data.name !== data.oldName) {
            const nameConflict = await db.select().from(radgroupreply).where(and(eq(radgroupreply.tenantId, targetTenantId), eq(radgroupreply.groupname, data.name))).limit(1);
...
export const deleteProfile = async (request, reply) => {
    try {
        ...
        await db.delete(radgroupreply).where(and(eq(radgroupreply.tenantId, targetTenantId), eq(radgroupreply.groupname, query.name)));
        await db.delete(radgroupcheck).where(and(eq(radgroupcheck.tenantId, targetTenantId), eq(radgroupcheck.groupname, query.name)));
