...
export const getUsers = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    const showDeleted = (request.query as any).showDeleted === 'true';
    ...
    const userList = await Promise.all(
      radcheckUsers.map(async (u) => {
        ...
        .where(
          and(
            eq(userOrganizations.username, u.username),
            eq(userOrganizations.tenantId, u.tenantId)
          )
        ).limit(1);
...
export const createUser = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    if (data.groupId) {
      const groupRes = await db.select().from(organizations).where(
        and(eq(organizations.id, data.groupId), eq(organizations.tenantId, targetTenantId))
      ).limit(1);
      ...
      const [activeCountRes] = await db.select({ count: sql<number>`cast(count(${userOrganizations.id}) as int)` })
        .from(userOrganizations)
      ...
      if (activeCount > 0) {
        const [suspendedCountRes] = await db.select({ count: sql<number>`cast(count(${userOrganizations.id}) as int)` })
          .from(userOrganizations)
          .innerJoin(radcheck, and(eq(userOrganizations.username, radcheck.username), eq(userOrganizations.tenantId, radcheck.tenantId), eq(radcheck.attribute, 'Cleartext-Password'), isNull(radcheck.deletedAt)))
          .where(and(
            eq(userOrganizations.organizationId, data.groupId),
            sql`EXISTS (
  ...
  try {
    ...
    if (data.groupId !== undefined) {
      ...
      if (data.groupId === null) {
        ...
        const existingMapping = await db.select().from(userOrganizations).where(
          and(eq(userOrganizations.tenantId, targetTenantId), eq(userOrganizations.username, username))
        ).limit(1);

        if (existingMapping.length > 0) {
          await db.update(userOrganizations).set({ organizationId: data.groupId }).where(
            and(eq(userOrganizations.tenantId, targetTenantId), eq(userOrganizations.username, username))
          );
        } else {
          await db.insert(userOrganizations).values({
        ...
        if (!data.profileName) {
          ...
          if (groupRes.length > 0 && groupRes[0]!.defaultProfile) {
            await db.update(radusergroup).set({ groupname: groupRes[0]!.defaultProfile }).where(
              and(eq(radusergroup.tenantId, targetTenantId), eq(radusergroup.username, username))
            );
          }
        }
...
export const deleteUser = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    const activeSession = await db.select().from(radacct).where(
      and(eq(radacct.tenantId, targetTenantId), eq(radacct.username, username), isNull(radacct.acctstoptime))
    ).limit(1);
...
export const restoreUser = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const user = request.user as any;
    const { username } = request.params as { username: string };
    const targetTenantId: string | null = user.tenantId || (request.query as any).tenantId || null;
...
export const permanentDeleteUser = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const user = request.user as any;
    const { username } = request.params as { username: string };
    const targetTenantId: string | null = user.tenantId || (request.query as any).tenantId || null;
