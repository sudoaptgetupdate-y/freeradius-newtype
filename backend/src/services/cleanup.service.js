        ...
        try {
            ...
            for (const tenant of tenantList) {
                ...
                const usersToDelete = await db.select({ username: radcheck.username })
                    .from(radcheck)
                    .where(and(eq(radcheck.tenantId, tenant.id), isNotNull(radcheck.deletedAt), sql `${radcheck.deletedAt} < ${cutoffDate.toISOString()}`));
                if (usersToDelete.length > 0) {
                    ...
                    for (const username of usernames) {
                        await db.delete(radcheck).where(and(eq(radcheck.tenantId, tenant.id), eq(radcheck.username, username)));
                        await db.delete(radusergroup).where(and(eq(radusergroup.tenantId, tenant.id), eq(radusergroup.username, username)));
                        await db.delete(userOrganizations).where(and(eq(userOrganizations.tenantId, tenant.id), eq(userOrganizations.username, username)));
                    }
                }
            }
