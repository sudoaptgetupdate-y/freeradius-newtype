...
async function cleanup() {
  ...
  for (const r of records) {
    ...
    if (r.attribute === "User-First-Name") data.firstName = r.value;
    if (r.attribute === "User-Last-Name") data.lastName = r.value;
    if (r.attribute === "User-Member-Id") data.memberId = r.value;
    if (r.attribute === "User-Citizen-Id") data.citizenId = r.value;
    if (r.attribute === "User-Email") data.email = r.value;
    if (r.attribute === "User-Phone") data.phone = r.value;
  }
