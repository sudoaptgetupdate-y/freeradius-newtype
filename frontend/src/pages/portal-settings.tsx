...
export default function PortalSettings() {
  ...
  const fetchSettings = async (targetTenantId: string) => {
    setLoading(true)
    try {
      const { data } = await api.get(`/portal/settings/${targetTenantId}`)
      setSettings({
          ...
          <Card className="border-border">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                ...
                <div className="flex items-center justify-between p-4 bg-muted/20 rounded-lg border border-border/50">
                  <div className="space-y-0.5">
                    <Label className="text-sm font-medium">Social Logins</Label>
                    <p className="text-xs text-muted-foreground">Enable LINE & Google auth options.</p>
                  </div>
                  <Switch
                ...
                <div className="space-y-4 pt-4 border-t border-border/50 animate-in fade-in zoom-in-95 duration-200">
                  <div className="bg-muted/20 p-4 rounded-lg border border-border/50 space-y-4">
                    <h4 className="text-sm font-semibold flex items-center gap-2">
                      ...
                      Google OAuth (Tenant Override)
                    </h4>
                    <p className="text-xs text-muted-foreground mt-1 mb-2">Leave blank to use the Global Control Panel settings.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label>Client ID</Label>
                        <Input 
                          value={settings.googleClientIdOverride}
                          onChange={(e) => setSettings({ ...settings, googleClientIdOverride: e.target.value })}
                      ...
                      <div className="space-y-2">
                        ...
                        <Input 
                          type="password"
                          value={settings.googleClientSecretOverride}
                          onChange={(e) => setSettings({ ...settings, googleClientSecretOverride: e.target.value })}
                          placeholder="Optional Override"
                        />
                      </div>
                    </div>
                  ...
                  <div className="bg-muted/20 p-4 rounded-lg border border-border/50 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input 
                          value={settings.lineChannelIdOverride}
                          onChange={(e) => setSettings({ ...settings, lineChannelIdOverride: e.target.value })}
                          placeholder="Optional Override"
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input 
                          type="password"
                          value={settings.lineChannelSecretOverride}
                          onChange={(e) => setSettings({ ...settings, lineChannelSecretOverride: e.target.value })}
                          placeholder="Optional Override"
                        />
                      </div>
                    </div>
